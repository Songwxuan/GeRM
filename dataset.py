...
if __name__ == '__main__':
    import time
    # time0 = time.time()
    # real_file = open('dataset_info/quadruped_data_info.json', 'r')
    # real_dict = json.load(real_file)
    # real_dataset = [RT1Dataset(real_dict[key]) for key in real_dict.keys()]
    # real_dataset = torch.utils.data.ConcatDataset(real_dataset)
    # time1 = time.time()
    # print('real dataset loaded, time: ', time1 - time0)

    # time0 = time.time()
    # sim_file = open('dataset_info/sim_quadruped_data_info.json', 'r')
    # sim_dict = json.load(sim_file)
    # sim_dataset = [RT1Dataset(sim_dict[key]) for key in sim_dict.keys()]
    # sim_dataset = torch.utils.data.ConcatDataset(sim_dataset)
    # time1 = time.time()
    # print('sim dataset loaded, time: ', time1 - time0)

    sim_file = open('dataset_info/sim_quadruped_data_info.json', 'r')
    sim_dict = json.load(sim_file)
    count = 0
    for key in sim_dict.keys():
        sim_dataset = RT1Dataset(sim_dict[key])
        count += 1
        if count > 0:
            break
